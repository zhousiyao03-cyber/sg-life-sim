...
| 夜归 | `FNightCommuteSystem` / `UNightCommuteSubsystem` | 农历七月深夜回家遇 13 楼空楼层电梯，玩家**主动抉择**：等下一趟（安全慢）/ 赶紧进去（省事但 55% 招事重扣理智）/ 走楼梯（最稳最累）；把恐怖从被动播报升级成主动博弈，可种子复现 |
...
跑测试：`Automation RunTests SGLifeSim`（headless `UnrealEditor-Cmd ... -nullrhi`），当前 **78 个全绿**。
